...
get_template_function_name_use_type() -> const std::string_view {
...
template <typename TargetType>
[[nodiscard]] constexpr auto
get_template_function_name_use_decay_type() -> const std::string_view {
...
template <class T>
[[nodiscard]] constexpr auto min_value(const T &lhs,
                                       const T &rhs) -> const T & {
...
struct fatal {
  template <class T> [[nodiscard]] inline auto operator()(const T &t) const {
...
template <class TExpr> struct throws_<TExpr, void> : op {
  constexpr explicit throws_(TExpr &&expr)
      : value_{[&expr] {
          try {
            expr();
            return false;
...
template <class TExpr> struct aborts_ : op {
  constexpr explicit aborts_(TExpr &&expr)
      : value_{[&expr]() -> bool {
          if (const auto pid = fork(); not pid) {
            expr();
            std::exit(0);
...
template <class TExpr> [[nodiscard]] constexpr auto throws(TExpr &&expr) {
  return detail::throws_{std::forward<TExpr>(expr)};
}
...
// }
template <class TExpr> [[nodiscard]] constexpr auto aborts(TExpr &&expr) {
  return detail::aborts_{std::forward<TExpr>(expr)};
}
#endif
